...
const ADB = {
  ...
  async saveProduct(p){
    ...
    if(p.id !== undefined && p.id !== null && p.id !== ""){
      ...
      const {id,...rest} = p;
      const {data,error} = await sb.from('products').insert(rest).select().single();
      if(error) throw error;
      return data;
    }
  },
  ...
  // ── Sunday ──
  async getSunday(){
    if(!isSupabaseConfigured){ try{return JSON.parse(localStorage.getItem('kt_sunday'))||{};}catch{return{};} }
    const {data} = await sb.from('sunday_special').select('*').eq('id',1).single();
    return data||{};
  },
  async saveSunday(productId, price, message){
    if(!isSupabaseConfigured){ localStorage.setItem('kt_sunday',JSON.stringify({productId,price,message}));return; }
    await sb.from('sunday_special').update({product_id:productId,price,message,updated_at:'now()'}).eq('id',1);
  },
};
...
async function saveProd(){
  ...
  try{
    if(isSupabaseConfigured){
      var savedRow = await ADB.saveProduct(p);
      if(savedRow){
...
function renderSpecialPage(){
  ...
  if(sp.image)     spImgData = sp.image;
  updateSpPreview();
...
function saveSpecial(){
  ...
  lsSet("sunday",{productId:pid,price:price,message:msg,image:img});
  toast("✅ Spécial Dimanche sove!","s");
}

var spImgData = "";
function handleSpecialImg(input){
  if(input.files && input.files[0]){
    var reader = new FileReader();
    reader.onload = function(e){
      spImgData = e.target.result;
      document.getElementById("sp-pv-img").innerHTML = '<img src="'+spImgData+'" style="width:100%;height:auto;border-radius:12px;">';
    };
